...
def clickdydx():
    ...
    c = Canvas(frame,scrollregion=(-500, -500, 500, 500),width=500,height=500, bg='black', yscrollcommand=v.set, xscrollcommand=h.set)
    ...
    if errorFlag:
        messagebox.showerror('Math Error', 'В пределах интегрирования ошибки. \nНапример sqrt(-2) или деление на ноль', parent=window)
    if anti:
        messagebox.showerror('Math Error', 'Верхний предел меньше нижнего!\nМожет, ты перепутал границы интегрирования?\n(красным выделена область с отрицательной плотность)', parent=window)
...
def clickdxdy():
    ...
    for x in np.arange(dx[1],dx[0],res/50):
        ...
        try:
            ...
            c.create_text(x*50/zoom, error, text=f"ERROR in X={x:.2f}",
              anchor=SE, fill="red")
            errorFlag=True
            error+=10
            continue
        # for y in np.arange(dy[1],dy[0]+1,res/50):
        if(dy[0]>=dy[1]):
            c.create_rectangle(x*50/zoom,-(dy[1]*50/zoom),x*50/zoom+10/zoom*res/10,-(dy[0]*50/zoom),fill="white",outline="white")
        else:
            anti=True
            c.create_rectangle(x*50/zoom,-(dy[1]*50/zoom),x*50/zoom+10/zoom*res/10,-(dy[0]*50/zoom),fill="red",outline="white")
        surface+=res/50*((dy[0])-(dy[1]))
        
    for x in np.arange(int(dx[1]),int(dx[0])+1):
            c.create_text(x*50/zoom, 0, text=f"X={int(x)}",
              anchor=SE, fill="red")
    c.create_line(-10000,0,10000,0,fill='green')
    c.create_line(0,-10000,0,10000,fill='green')
    if errorFlag:
...
Scala = Scale(from_=0.25, to=5, orient=HORIZONTAL,length=150,resolution=0.25,label="Разрешение")
Scala.grid(row=5,column=0)
Scala.set(3)
# label = ttk.Label(text="Маштаб",anchor="w", justify="left")
# label.grid(sticky="W",row=6,column=0)

Scala2 = Scale(from_=0.5, to=5, orient=HORIZONTAL,length=150,resolution=0.25,label="Маштаб")
